...
async function parseMediaFromRequest(c: any): Promise<ParsedMedia> {
  ...
  // ── MULTIPART FORM DATA (video/image file uploads) ──
  if (contentType.includes("multipart/form-data")) {
    ...
    if (!file) {
      throw {
        errorCode: "E_NO_FILE",
        message: "No file found in form data. Send a file with field name 'file'.",
        guidance: [
          "Use multipart/form-data with a field named 'file'",
          "Accepted image types: JPEG, PNG, WebP, GIF",
          "Accepted video types: MP4, MOV, AVI, WebM",
          "Or send JSON body with 'image' (base64) or 'video' (base64)",
        ],
      };
    }

    const mimeType = file.type;
    const isVideo = SUPPORTED_VIDEO_TYPES.includes(mimeType) || mimeType.startsWith("video/");
    ...
    if (!isVideo && !isImage) {
      throw {
        ...
        guidance: [
          ...
          "Rename file or convert to a supported format",
        ],
      };
    }
    ...
    if (isVideo) {
      ...
      const tempPath = path.join(tempDir, `preventiq_${Date.now()}.${ext}`);
      ...
      if (sizeMB > MAX_VIDEO_SIZE_MB) {
        throw {
          errorCode: "E_VIDEO_TOO_LARGE",
          message: `Video is ${sizeMB.toFixed(1)}MB — maximum is ${MAX_VIDEO_SIZE_MB}MB.`,
          guidance: [
            "Trim the video to under 2 minutes",
            "Compress using a lower resolution (720p is fine)",
            "Use MP4 with H.264 for smallest file size",
            "On iPhone: Settings > Camera > Record Video > 720p HD",
          ],
        };
      }

      fs.writeFileSync(tempPath, Buffer.from(arrayBuffer));
      console.log(`[Vision] Video temp: ${tempPath} (${sizeMB.toFixed(1)}MB, ${mimeType})`);

      return {
        media: { type: "video", data: tempPath, mimeType },
    ...
    return {
      media: { type: "image", data: base64, mimeType },
      additionalContext: additionalContext ?? undefined,
    };
  }
  ...
  try {
    ...
    throw {
      ...
      guidance: [
        "Send JSON with 'image' (base64 string) or 'video' (base64 string)",
        "Or use multipart/form-data with a 'file' field",
        "Include 'mimeType' field (e.g. 'image/jpeg', 'video/mp4')",
      ],
    };
  }

  if (!body.image && !body.video) {
    throw {
      errorCode: "E_NO_MEDIA",
      message: "Request must include 'image' or 'video' field.",
      guidance: [
        "JSON body: { \"image\": \"<base64>\", \"mimeType\": \"image/jpeg\" }",
        "JSON body: { \"video\": \"<base64>\", \"mimeType\": \"video/mp4\" }",
        "Or use multipart/form-data with a 'file' field for direct uploads",
        "Include optional 'additionalContext' for symptom descriptions",
      ],
    };
  }
  ...
  if (body.video) {
    ...
    const ext = mimeType.split("/")[1]?.replace("quicktime", "mov") || "mp4";
    const tempPath = path.join(os.tmpdir(), `preventiq_${Date.now()}.${ext}`);

    const buffer = Buffer.from(body.video, "base64");
    ...
    if (sizeMB > MAX_VIDEO_SIZE_MB) {
      throw {
        ...
        guidance: [
          "Trim the video to under 2 minutes",
          "Compress or use 720p resolution",
        ],
      };
    }

    fs.writeFileSync(tempPath, buffer);
    console.log(`[Vision] Base64 video → temp: ${tempPath} (${sizeMB.toFixed(1)}MB)`);
  ...
  if (typeof body.image !== "string" || body.image.length < 100) {
    throw {
      ...
      guidance: [
        ...
        "Test by decoding: Buffer.from(base64, 'base64') should produce valid image bytes",
      ],
    };
  }
...
function formatPipelineResponse(pipeline: AnalysisPipeline, analysisType: string) {
  ...
  if (pipeline.framesAnalyzed !== undefined) {
    base.framesAnalyzed = pipeline.framesAnalyzed;
  }
...
async function handleVisionRequest(
  ...
  try {
    const { media, additionalContext, tempFilePath } = await parseMediaFromRequest(c);
    ...
    if (tempFile && fs.existsSync(tempFile)) {
      ...
      console.log(`[Vision] Cleaned temp: ${tempFile}`);
    }
  }
}
