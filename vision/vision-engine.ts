...
export type MediaInput = {
  type: "image" | "video";
  data: string;          // base64 for images, file path for videos
  mimeType: string;
};
...
export async function uploadVideoToGemini(filePath: string, mimeType: string): Promise<string> {
  ...
  console.log(`[VisionEngine] Uploading video: ${fileName} (${mimeType})`);

  const uploadResult = await fileManager.uploadFile(filePath, {
    mimeType,
    displayName: `preventiq_scan_${Date.now()}`,
  });

  let file = uploadResult.file;
  console.log(`[VisionEngine] Upload started, file name: ${file.name}, state: ${file.state}`);
  ...
  while (file.state === FileState.PROCESSING) {
    if (Date.now() - startTime > maxWait) {
      throw new Error("Video processing timed out after 2 minutes. Try a shorter clip.");
    }

    console.log(`[VisionEngine] Video still processing... (${Math.round((Date.now() - startTime) / 1000)}s)`);
  ...
  if (file.state === FileState.FAILED) {
    throw new Error(`Video processing failed: ${file.name}. The file may be corrupted or in an unsupported format.`);
  }

  console.log(`[VisionEngine] Video ready: ${file.uri}`);
  return file.uri;
}
...
export async function runAnalysisPipeline(
  ...
  if (media.type === "video") {
    try {
      const videoUri = await uploadVideoToGemini(media.data, media.mimeType);
      mediaParts = [{ fileData: { mimeType: media.mimeType, fileUri: videoUri } }];
      framesAnalyzed = -1; // Gemini handles frame extraction internally
    } catch (err: any) {
      return {
        success: false,
        finalResult: buildErrorResult("VIDEO_UPLOAD_FAILED", err.message, [
          "Ensure the video is under 2 minutes long",
          "Supported formats: MP4, MOV, AVI, WEBM",
          "Check that the file is not corrupted",
          "Try recording a new, shorter clip",
        ]),
        passes: [],
        backtrackLog: [],
    ...
    mediaParts = [{ inlineData: { mimeType: media.mimeType, data: media.data } }];
  }
  ...
  if (!config.forceType) {
    ...
    if (!triage) {
      ...
      if (!enhancedResult.found) {
        ...
        return {
          success: false,
          finalResult: buildSubjectNotFoundResult(config.analysisType, triage, media.type),
          passes,
          backtrackLog,
          totalDurationMs: Date.now() - pipelineStart,
      ...
      if (!recoveryResult.usable) {
        return {
          success: false,
          finalResult: buildQualityErrorResult(triage, media.type),
          passes,
          backtrackLog,
          totalDurationMs: Date.now() - pipelineStart,
          mediaType: media.type,
  ...
  const mainPrompt = buildMainPrompt(config, media.type);
  ...
  if (!mainResult.result) {
    ...
    const simplifiedPrompt = buildSimplifiedPrompt(config, media.type);
...
async function runEnhancedDetection(
  ...
  const prompt = `
The initial scan did not detect the expected subject. Look MORE CAREFULLY at this ${config.analysisType === "fundus" ? "image/video" : "image/video"}.
...
- For video: scan ALL frames, not just the first one
...
Return JSON:
{
  ...
  "bestFrameDescription": "For video: describe the most relevant frame. For image: describe the most relevant region.",
...
async function runQualityRecovery(
  ...
  const prompt = `
The image/video quality was flagged as UNUSABLE with these issues: ${triage.qualityIssues?.join(", ")}.

Despite the poor quality, try to extract ANY useful clinical information. Even partial observations are valuable.

For video: identify the BEST frame(s) and analyze those.
For images: focus on the highest-quality region.
...
4. For video: were all relevant frames considered?
...
function buildMainPrompt(config: AnalysisConfig, mediaType: "image" | "video"): string {
  const mediaInstruction = mediaType === "video"
    ? `
VIDEO ANALYSIS INSTRUCTIONS:
- Analyze ALL frames systematically. Do not just look at the first frame.
- For fundus: the patient may be moving — find the clearest frame showing the retina.
- For skin: track how the condition appears across different angles/lighting in the video.
- Note any temporal changes visible across frames (e.g., pulsation, movement, color shifts).
- In your findings, reference specific moments if relevant (e.g., "mid-video shows clearest view of...").
    `.trim()
    : "";
...
function buildSimplifiedPrompt(config: AnalysisConfig, mediaType: "image" | "video"): string {
  return `
Analyze this medical ${mediaType} for ${config.analysisType} screening.
${config.profileBlock}
...
function buildSubjectNotFoundResult(
  ...
  const guidance: Record<string, string[]> = {
    fundus: [
      ...
      mediaType === "video" ? "Hold the camera steady for at least 3 seconds once the retina is in view" : "",
    ].filter(Boolean),
    skin: [
      ...
      mediaType === "video" ? "Slowly pan across the affected area to show the full extent" : "",
    ].filter(Boolean),
    general: [
      "Ensure the health-related subject is clearly visible and in focus",
      "Use good lighting and hold the camera steady",
      mediaType === "video" ? "Record for at least 5 seconds" : "",
    ].filter(Boolean),
  };
  ...
  return {
    ...
    message: `Could not detect a valid ${analysisType} subject in the uploaded ${mediaType}. ${detectedInstead}`,
...
function buildQualityErrorResult(triage: any, mediaType: "image" | "video"): any {
  return {
    error: "QUALITY_UNUSABLE",
    errorCode: "E_BAD_QUALITY",
    message: `The ${mediaType} quality is too poor for reliable analysis.`,
    ...
    recaptureGuidance: [
      ...
      mediaType === "video" ? "Record in at least 720p resolution" : "Use the highest resolution camera setting available",
    ],
