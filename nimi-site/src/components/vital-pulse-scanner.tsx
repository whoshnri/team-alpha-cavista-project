...
export function VitalPulseScanner({ onComplete }: VitalPulseScannerProps) {
        ...
        <div className="flex flex-col items-center justify-center h-full w-full">
            <div className="space-y-8 flex flex-col items-center">
                <div className="w-12 h-12 border border-border flex items-center justify-center rounded-lg">
                    <HeartPulse className="w-6 h-6 text-text-primary" />
                </div>
                <button
            ...
            {isMobile === false && (
                <p className="section-label mt-8 text-destructive">
                    Smartphone Required
                </p>
            )}
        </div>
        ...
        return (
            <div className="flex flex-col items-center justify-center h-full w-full gap-12">
                <div className="relative flex items-center justify-center">
                    <div className="absolute w-44 h-44 rounded-full border border-border loading-pulse" />
                    <span className="text-8xl font-bold tabular-nums text-text-primary tracking-tighter metric-value">
                        {countdown}
                    </span>
                </div>
            ...
            <div className="space-y-2">
                ...
                <div className="bg-surface border border-border p-3 rounded-lg space-y-2">
                    {errorMessages.map((msg, i) => (
                        <p key={i} className="text-[11px] text-text-muted leading-relaxed">{msg}</p>
                    ))}
                </div>
            </div>
            <Button
                ...
                variant="outline"
                className="rounded-lg px-8 border-border hover:bg-surface-raised"
            >
                Try Again
            </Button>
        </div>
    ...
    const renderUnsupported = () => (
        <div className="flex flex-col items-center text-center gap-6 p-4 animate-in fade-in slide-in-from-top-4 duration-500">
            <div className="p-4 bg-accent-blue/10 rounded-full">
                <Smartphone className="w-12 h-12 text-accent-blue" />
            </div>
            <div className="space-y-2">
                ...
                <div className="bg-surface border border-border p-3 rounded-lg">
                    {errorMessages.map((msg, i) => (
                        <p key={i} className="text-[11px] text-text-muted">{msg}</p>
                    ))}
                </div>
            </div>
            <Button
    ...
    const renderComplete = () => {
        ...
        return (
            <div className="flex flex-col items-center text-center gap-8 w-full max-w-sm">
                <div className="space-y-2">
                    <h1 className="text-2xl font-bold tracking-tight text-text-primary uppercase">Scan Complete</h1>
                ...
                <div className="w-full card p-8">
                    <div className="flex flex-col items-center gap-2 mb-8">
                        ...
                        <div className="flex items-baseline gap-2">
                            <span className="text-8xl font-bold text-text-primary tracking-tighter metric-value leading-none">
                    ...
                    <div className="grid grid-cols-2 gap-8 border-t border-border pt-8">
                        <div>
                            ...
                            <p className="text-2xl font-bold text-text-primary tracking-tight metric-value">
                        ...
                        <div>
                            ...
                            <div className="flex items-center justify-center gap-2">
                                <div className={cn(
                                    ...
                                    heart_rate.signal_quality === 'good' ? "bg-accent-blue" :
                                        heart_rate.signal_quality === 'fair' ? "bg-accent-blue/40" : "bg-destructive"
                                )} />
                ...
                <div className="grid grid-cols-2 gap-4 w-full">
                    <div className="bg-surface border border-border rounded-lg p-4 flex flex-col items-center">
                        ...
                        <span className="text-lg font-bold text-text-primary metric-value">{heart_rate.median_ibi_ms}ms</span>
                    </div>
                    <div className="bg-surface border border-border rounded-lg p-4 flex flex-col items-center">
                        <span className="section-label mb-1">Peak Count</span>
                        <span className="text-lg font-bold text-text-primary metric-value">{heart_rate.peaks_after_outlier_rejection}</span>
                    </div>
                </div>
                ...
                <div className="flex flex-col gap-4 w-full mt-4">
                    ...
                    <Accordion type="single" collapsible className="w-full">
                        <AccordionItem value="item-1" className="border-none">
                            ...
                            <AccordionContent>
                                <div className="bg-surface border border-border rounded-lg p-4 text-left relative group">
                                    <button
                                    ...
                                    <pre className="text-[10px] text-text-muted whitespace-pre-wrap break-words font-mono overflow-y-auto max-h-32 custom-scrollbar">
    ...
    return (
        <div className="relative w-full max-w-[420px] aspect-[9/14] bg-background border border-border rounded-lg overflow-hidden flex flex-col items-center justify-between p-12 select-none shadow-none">
